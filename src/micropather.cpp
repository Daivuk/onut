...
int MicroPather::Solve(void* startNode, void* endNode, MP_VECTOR< void* >* path, float* cost)
{
    ...
    *cost = 0.0f;
    ...
    PathNode* closestClosed = nullptr;
    float closestCost = 10000000.0f;
    ...
    while (!open.Empty())
    {
        ...
        else
        {
            ...
            if (node->estToGoal < closestCost)
            {
                closestCost = node->estToGoal;
                closestClosed = node;
            }
        }
    }
    ...
    if (closestClosed)
    {
        GoalReached(closestClosed, startNode, closestClosed->state, path);
        *cost = closestClosed->costFromStart;
    }
    else
    {
        if (pathCache) {
