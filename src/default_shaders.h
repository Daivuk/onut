...
static const char* SHADER_SRC_3D_PNCT_VS = ""
    ...
    "    float3 normal = normalize(mul(oModel, float4(inNormal, 0.0)).xyz);\n"
    "    float3 worldPos = mul(oModel, float4(inPosition, 1.0)).xyz;\n"
    "    outColor.rgb = ambient + lerp(inColor.rgb * sunColor * (dot(sunDir, normal) * 0.5 + 0.5), inColor.rgb, 0);\n"
    "    outColor.rgb += LightPass(worldPos, normal, l0_posRadius.xyz, l0_posRadius.w, l0_color);\n"
    "    outColor.rgb += LightPass(worldPos, normal, l1_posRadius.xyz, l1_posRadius.w, l1_color);\n"
    "    outColor.rgb += LightPass(worldPos, normal, l2_posRadius.xyz, l2_posRadius.w, l2_color);\n"
    "    outColor.a = inColor.a;\n"
    "    outTexCoord = inTexCoord;\n"
    "}\n"
"";

static const char* SHADER_SRC_3D_PNC_VS = ""
    "extern float3 sunDir;\n"
    "extern float3 sunColor;\n"
    "extern float3 ambient;\n"
    "extern float4 l0_posRadius;\n"
...
static const char* SHADER_SRC_3D_PNT_VS = ""
    "extern float3 sunDir;\n"
    "extern float3 sunColor;\n"
    "extern float3 ambient;\n"
    "extern float4 l0_posRadius;\n"
    ...
    "    float3 normal = normalize(mul(oModel, float4(inNormal, 0.0)).xyz);\n"
    "    float3 worldPos = mul(oModel, float4(inPosition, 1.0)).xyz;\n"
    "    outColor.rgb = ambient + sunColor * (dot(sunDir, normal) * 0.5 + 0.5);\n"
    "    outColor.rgb += LightPass(worldPos, normal, l0_posRadius.xyz, l0_posRadius.w, l0_color);\n"
    "    outColor.rgb += LightPass(worldPos, normal, l1_posRadius.xyz, l1_posRadius.w, l1_color);\n"
    "    outColor.rgb += LightPass(worldPos, normal, l2_posRadius.xyz, l2_posRadius.w, l2_color);\n"
    "    outColor.a = 1.0;\n"
...
static const char* SHADER_SRC_3D_PN_VS = ""
    "extern float3 sunDir;\n"
    "extern float3 sunColor;\n"
    "extern float3 ambient;\n"
    "extern float4 l0_posRadius;\n"
...
static const char* SHADER_SRC_3D_PNCTW_VS = ""
    "extern float3 sunDir;\n"
    "extern float3 sunColor;\n"
    "extern float3 ambient;\n"
    "extern float4 l0_posRadius;\n"
    ...
    "extern float4 l2_posRadius;\n"
    "extern float3 l2_color;\n"
    "extern matrix bones[64];\n"
    "\n"
    "input float3 inPosition;\n"
    "input float3 inNormal;\n"
    "input float4 inColor;\n"
    "input float2 inTexCoord;\n"
    "input float2 inWeight0;\n"
    "input float2 inWeight1;\n"
    "input float2 inWeight2;\n"
    "input float2 inWeight3;\n"
    "\n"
    "output float4 outColor;\n"
    "output float2 outTexCoord;\n"
    "\n"
    ...
    "void main()\n"
    "{\n"
    "    matrix boneTransform = bones[(int)inWeight0.x] * inWeight0.y;\n"
    "    boneTransform += bones[(int)inWeight1.x] * inWeight1.y;\n"
    "    boneTransform += bones[(int)inWeight2.x] * inWeight2.y;\n"
    "    boneTransform += bones[(int)inWeight3.x] * inWeight3.y;\n"
    "    float4 posL = mul(float4(inPosition, 1.0), boneTransform);\n"
    "    oPosition = mul(posL, oViewProjection);\n"
    "    float3 normL = mul(float4(inNormal, 0.0), boneTransform).xyz;\n"
    "    float3 normal = normalize(mul(oModel, float4(normL, 0.0)).xyz);\n"
    "    float3 worldPos = mul(oModel, float4(inPosition, 1.0)).xyz;\n"
    "    outColor.rgb = ambient + lerp(inColor.rgb * sunColor * (dot(sunDir, normal) * 0.5 + 0.5), inColor.rgb, 0);\n"
    "    outColor.rgb += LightPass(worldPos, normal, l0_posRadius.xyz, l0_posRadius.w, l0_color);\n"
    "    outColor.rgb += LightPass(worldPos, normal, l1_posRadius.xyz, l1_posRadius.w, l1_color);\n"
...
static const char* SHADER_SRC_3D_PNCW_VS = ""
    "extern float3 sunDir;\n"
    "extern float3 sunColor;\n"
    "extern float3 ambient;\n"
    "extern float4 l0_posRadius;\n"
...
static const char* SHADER_SRC_3D_PNTW_VS = ""
    "extern float3 sunDir;\n"
    "extern float3 sunColor;\n"
    "extern float3 ambient;\n"
    "extern float4 l0_posRadius;\n"
...
static const char* SHADER_SRC_3D_PNW_VS = ""
    "extern float3 sunDir;\n"
    "extern float3 sunColor;\n"
    "extern float3 ambient;\n"
    "extern float4 l0_posRadius;\n"
...
static const char* SHADER_SRC_3D_CT_PS = ""
    "Texture0 texDiffuse;\n"
    "\n"
...
static const char* SHADER_SRC_3D_C_PS = ""
    "input float4 inColor;\n"
    "\n"
    "void main()\n"
    "{\n"
...
static const char* SHADER_GBUFFER_ALPHA_TEST_PS = ""
    ...
    "    float4 normal = texNormal(inTexCoord);\n"
    "    float4 material = texMaterial(inTexCoord);\n"
    "    oColor0 = float4(diffuse.rgb, 1.0);\n"
    "    oColor1 = float4(inNormal.rgb * 0.5 + 0.5, 1.0);\n"
    "    oColor2 = float4(inDepth.x / inDepth.y, 0.0, 0.0, 1.0);\n"
    "}\n"
"";
