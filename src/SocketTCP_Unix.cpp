...
SocketTCPRef SocketTCP::listen(int port)
{
    auto pRet = shared_ptr<SocketTCP_Unix>(new SocketTCP_Unix());
    auto pRaw = pRet.get();
    ...
    if (pRaw->m_socket < 0)
    {
        cout << "MM listen socket creation failed" << endl;
        return nullptr;
    }
    ...
    {
        cout << "MM listen bind failed" << endl;
        return nullptr;
    }
...
SocketTCPRef SocketTCP::connect(const std::string& ip, int port)
{
    return nullptr;
}
...
SocketTCPRef SocketTCP_Unix::accept()
{
    // Accept a client socket
    auto clientSocket = ::accept(m_socket, NULL, NULL);
    if (clientSocket < 0)
    {
        cout << "MM accept failed" << endl;
        return nullptr;
    }

    auto pClientSocket = shared_ptr<SocketTCP_Unix>(new SocketTCP_Unix());
    pClientSocket->m_socket = clientSocket;
    return pClientSocket;
}

bool SocketTCP_Unix::recv(vector<uint8_t> &out)
{
