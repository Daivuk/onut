...
#define myTex2D(s,p) texDiffuse.Sample(s,p)

cbuffer CRTBuffer : register(b0)
{
    float2 screen_size;
    float2 pixel;
}

static const float2 aspect = float2(1.0f, screen_size.y / screen_size.x);

// [dsl] TODO
//float4 p0 : register(c0);
//float4 p1 : register(c1);
//static const float2 p0 = float2(256, 256);
//static const float2 p1 = float2(1.0 / p0.x, 1.0 / p0.y);
...
#       define TEX2D(c) pow(myTex2D(samplerState, (c)), CRTgamma)
#else
#       define TEX2D(c) myTex2D(samplerState, (c))
#endif
...
float4 main(PSInput input) : SV_TARGET
{
    ...
    c0 = AdvancedCRTPass(c0, input.texCoord);
    return c0;
}
